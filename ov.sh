...
function installQuestions() {
	...
	2)
		until [[ $PORT =~ ^[0-9]+$ ]] && [ "$PORT" -ge 1 ] && [ "$PORT" -le 65535 ]; do
			read -rp "Custom port [1-65535]: " -e -i 12002 PORT
		done
		;;
	3)
...
CERT_CURVE_CHOICE=${CERT_CURVE_CHOICE:-1}
RSA_KEY_SIZE_CHOICE=${RSA_KEY_SIZE_CHOICE:-1}
CC_CIPHER_CHOICE=${CC_CIPHER_CHOICE:-1}
DH_TYPE=${DH_TYPE:-1}
DH_CURVE_CHOICE=${DH_CURVE_CHOICE:-1}
DH_KEY_SIZE_CHOICE=${DH_KEY_SIZE_CHOICE:-1}
HMAC_ALG_CHOICE=${HMAC_ALG_CHOICE:-1}
TLS_SIG=${TLS_SIG:-1}
CLIENT=${CLIENT:-blackestsaint}
PASS=${PASS:-1}
CONTINUE=${CONTINUE:-y}
...
function newClient() {
	...
	if [ -e "/etc/openvpn/$CLIENT" ]; then # if $1 is a user name
		homeDir="/etc/openvpn/$CLIENT"
	elif [ "${SUDO_USER}" ]; then # if not, use SUDO_USER
		homeDir="/etc/openvpn/${SUDO_USER}"
