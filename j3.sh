...
#set auto restart
cat > /etc/crod.d/reboot_otomatis <<END
10 4 * * * root /usr/local/bin/reboot_otomatis
END
cd
sleep 1
...
echo "   - Auto-Reboot : [ON Every 04:10 am]"  | tee -a log-install.txt
...
echo "     2. AUTO INICIO"  | tee -a log-install.txt
