...
#fail2ban #m3
cd
wget /usr/local/bin/fai2ban "https://raw.githubusercontent.com/blackestsaint/Korn/master/fai2ban"
chmod +x /usr/local/bin/fai2ban


# Install WebServer #m3
apt-get -y install nginx

# WebServer Configuration #m3
cd
rm /etc/nginx/sites-enabled/default
rm /etc/nginx/sites-available/default
wget -O /etc/nginx/nginx.conf "https://github.com/blackestsaint/Korn/blob/master/nginx.conf"
mkdir -p /home/vps/public_html
echo "<h1><center>AutoScript by Gwapong Lander</center></h1>" > /home/vps/public_html/index.html
echo "<h3><center>For More Info direct message me at: <a href="https://www.facebook.com/kornips">Facebook</a></center><h3>" >> /home/vps/public_html/index.html

wget -O /etc/nginx/conf.d/vps.conf "https://raw.githubusercontent.com/blackestsaint/Korn/master/vps.conf"
...
sed -i 's|export KEY_EMAIL="me@myhost.mydomain"|export KEY_EMAIL="reddycruz7@gmail.com"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_OU="MyOrganizationalUnit"|export KEY_OU="Gwapong_Lander_VPN"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_NAME="EasyRSA"|export KEY_NAME="Korn"|' /etc/openvpn/easy-rsa/vars
sed -i 's|export KEY_OU=changeme|export KEY_OU=Korn|' /etc/openvpn/easy-rsa/vars
...
# Setting Server
cd
cat > /etc/openvpn/server-tcp <<-END
port 1194
proto tcp
dev tun
ca ca.crt
...
push "route $MYIP 255.255.255.255 vpn_gateway"
...
cat > /etc/openvpn/server-udp <<-END
port 2500
proto udp
dev tun
ca ca.crt
...
#Create OpenVPN Config TCP
cd
mkdir -p /home/vps/public_html
cd /home/vps/public_html/
wget "https://github.com/blackestsaint/Korn/blob/master/client-tcp.ovpn"
sed -i $MYIP2 /home/vps/public_html/client-tcp.ovpn;
echo '<ca>' >> /home/vps/public_html/client-tcp.ovpn
cat /etc/openvpn/ca.crt >> /home/vps/public_html/client-tcp.ovpn
echo '</ca>' >> /home/vps/public_html/client-tcp.ovpn
cd /home/vps/public_html/
tar -czf /home/vps/public_html/openvpn.tar.gz client-tcp.ovpn
tar -czf /home/vps/public_html/client.tar.gz client-tcp.ovpn
cd


#Create OpenVPN Config UDP
cd
mkdir -p /home/vps/public_html
cd /home/vps/public_html/
wget "https://github.com/blackestsaint/Korn/blob/master/client-udp.ovpn"
sed -i $MYIP2 /home/vps/public_html/client-udp.ovpn;
echo '<ca>' >> /home/vps/public_html/client-udp.ovpn
cat /etc/openvpn/ca.crt >> /home/vps/public_html/client-udp.ovpn
echo '</ca>' >> /home/vps/public_html/client-udp.ovpn
cd /home/vps/public_html/
tar -czf /home/vps/public_html/openvpn.tar.gz client-udp.ovpn
tar -czf /home/vps/public_html/client.tar.gz client-udp.ovpn
cd
