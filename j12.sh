...
#Start Up Command v2 (crontab)   ##m8
cd
...
@reboot root /etc/adm-lite/fff
10 4 * * * root /usr/local/bin/lander-reboot
@reboot root /usr/local/bin/aliasme" >> /etc/crontab
...
# Torrent and Anti-abuse Firewall  ##m8
cd
wget -O /etc/adm-lite/fff "https://raw.githubusercontent.com/blackestsaint/Korn/master/fff"
chmod +x /etc/adm-lite/fff
sleep 1
...
#install squid
apt-get install -y wget && wget https://raw.githubusercontent.com/blackestsaint/Korn/master/squid.4921.sh && chmod +x squideb.sh && ./squideb.sh
...
echo "  ★ Auto Configure Firewall every reboot  " | tee -a log-install.txt | lolcat
...
Summary:
Autamated Features:
1. Auto delete expired user account
2. Auto restart server every 04:10am
3. Auto disconnect multilogin users.
4. Auto configure firewall every reboot
5. Auto Reconnect features
6. Server Auto clear cache every restart

Tweaks:
1. Firewall for Torrent, Port Scanning, Ddos and Brute force attack
2. TCP Tweak for speed.
3. Squid Cache


Pre-installed:
1. Squid version 4.9.2
2. Privoxy
3. Dropbear
4. Over-HTTP-Proxy 
5. BADVPN
...
[ Not installed ] but included in Main Menu
1. Openvpn
2. SSL Tunnrel
3. Shadow Socks
4. Python Socks

" | tee -a log-install.txt | lolcat
