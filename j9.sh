...
echo "   - Auto-Reboot : [OFF] Go to menu to configure"  | tee -a log-install.txt  | lolcat
 echo "Server Information"  | tee -a log-install.txt | lolcat
echo "   - IPv6        : [OFF]"  | tee -a log-install.txt echo  | lolcat
echo ""  | tee -a log-install.txt
...
echo "   - Over-HTTP-Puncher for SSH : 8899,8899,7799,7798 "  | tee -a log-install.txt | lolcat
echo "   - TCP SPEED : ON: Experimental Only"  | tee -a log-install.txt
echo ""  | tee -a log-install.txt | lolcat
echo "   - Auto Clear Cache : ON: Clear Cache every Server restart."  | tee -a log-install.txt | lolcat
echo "   - Nginx       :  ON: 80"  | tee -a log-install.txt | lolcat
echo "   - Webmin   :  ON: http://$MYIP:10000/"  | tee -a log-install.txt | lolcat
echo "   - Vnstat     :   ON: http://$MYIP/vnstat/"  | tee -a log-install.txt | lolcat
echo ""  | tee -a log-install.txt | lolcat

echo "OHP: For Advance User: "  | tee -a log-install.txt | lolcat
echo "- OHP Port: 8899= Squid Port: 61790 |SSHD Port:22"  | tee -a log-install.txt | lolcat
echo "- OHP Port: 8898= Squid Port: 3993 |SSHD Port:333"  | tee -a log-install.txt | lolcat
echo "- OHP Port: 7799= Privoxy Port: 7190 |SSHD Port:22"  | tee -a log-install.txt | lolcat
echo "- OHP Port: 7798= Privoxy Port: 2256 |SSHD Port:333"  | tee -a log-install.txt | lolcat
echo ""  | tee -a log-install.txt

echo "   To display list of commands: menu"  | tee -a log-install.txt | lolcat
echo ""  | tee -a log-install.txt

echo "   To read this again type this: cat /root/log-install.txt"  | tee -a log-install.txt | lolcat
echo ""  | tee -a log-install.txt

echo "   Explanation of scripts and VPS setup" | tee -a log-install.txt | lolcat
echo "   Direct Messege me at www.facebook.com/kornips"  | tee -a log-install.txt | lolcat
echo ""  | tee -a log-install.txt
